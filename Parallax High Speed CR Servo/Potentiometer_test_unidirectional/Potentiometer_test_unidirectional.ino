...
void loop() {
  ...
  int potread = analogRead(pot); //declares a variable "potread" to be the values coming from A0
    ...
    int value = map (potread, 0, 1023, 1500, 1700); //maps the values from A0 to the range of the servo into a variable called "value"
   Serial.println (value); //shows us the numbers for "value"

   
  if (value <= 1550) //in case the potentiometer signal fluctuates
  {
  ...
  else if (value >= 1690) //in case the potentiometer signal fluctuates
  {
    ...
    Serial.println ("full throttle"); //the word "full throttle" will show up on the serial monitor if value is greater than 1690
  }
